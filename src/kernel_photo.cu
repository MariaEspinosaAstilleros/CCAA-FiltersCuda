...
/*Kernel*/
__global__ void kernel_sobel_photo(char* src_img, char* dst_img, int width_img , int height_img){
    ...
    if(num_col < (width_img - 1) && num_row < (height_img - 1)) {
        char grad_x = ((src_img[index] * gx[0][2]) + (2 * (src_img[index] * gx[1][2])) + (src_img[index] * gx[2][2])) - 
        ((src_img[index] * gx[0][0]) + (2 * (src_img[index] * gx[1][0])) + (src_img[index] * gx[2][0]));

        char grad_y = ((src_img[index] * gy[2][0]) + (2 * (src_img[index] * gy[2][1])) + (src_img[index] * gy[2][2])) - 
        ((src_img[index] * gy[0][0]) + (2 * (src_img[index] * gy[0][1])) + (src_img[index] * gy[0][2]));
...
void sobelFilterPhoto(cv::Mat *src_img){
    ...
    dim3 numBlocks(iDivUp(src_img->rows, BLOCK_SIZE), iDivUp(src_img->cols, BLOCK_SIZE));
    ...
    //call kernel
    kernel_sobel_photo<<<numBlocks,threadsPerBlock>>>(dev_src, dev_sobel, src_img->step1(), src_img->rows);
